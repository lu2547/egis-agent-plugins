...
# egis-agents 集成适配：
# 在 egis-agents 中，scripts/ 位于 core/tools/pptmaster/ 下，
# 但真实 templates/layouts/ 位于外部 skill 目录（由 PPT_MASTER_SKILL_DIR 环境变量指定）。
# 按环境变量优先、脑补默认“scripts 父目录”原生逻辑的顺序解析。
_ENV_SKILL_DIR = _os.getenv("PPT_MASTER_SKILL_DIR", "")
SKILL_DIR = Path(_ENV_SKILL_DIR).resolve(
) if _ENV_SKILL_DIR else SCRIPT_DIR.parent
        ...
        for tid in index:
            if tid in extracted:
                ...
                if (template_dir / "design_spec.md").exists():
                    try:
                        all_extras[tid] = _extract_entry(
                            tid, template_dir)["extras"]
