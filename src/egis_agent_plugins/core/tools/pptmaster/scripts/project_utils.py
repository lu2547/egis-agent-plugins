...
def _read_readme_metadata(project_path: Path) -> Dict[str, str]:
    """从项目根目录的 README.md 里读取元数据。

    README 格式由 project_manager.py init 写入：
        # {project_name}
    ...
    Returns:
        包含 name/format/date 键的字典（缺失字段为空字符串）。
    """
    meta = {'name': '', 'format': '', 'date': ''}
    ...
    m = re.search(r'^-\s*Canvas format:\s*(\S+)', content, flags=re.MULTILINE)
    if m:
        meta['format'] = m.group(1).strip()
    m = re.search(r'^-\s*Created:\s*(\d{8})', content, flags=re.MULTILINE)
    if m:
...
def parse_project_name(dir_name: str) -> Dict[str, str]:
    """浅解析目录名，仅返回原始字符串作为 name。

    新命名规范下，格式/日期由 README 元数据提供，不再从目录名推断。
    详细信息请调用 get_project_info。
    """
    return {
        'name': dir_name,
        'format': 'unknown',
...
def get_project_info(project_path: str) -> Dict:
    ...
    # 从 README 元数据覆写 name/format/date（新命名规范下为唯一来源）
    readme_meta = _read_readme_metadata(project_path)
    if readme_meta['name']:
        info['name'] = readme_meta['name']
    if readme_meta['format']:
        normalized = normalize_canvas_format(readme_meta['format'])
        info['format'] = normalized
        if normalized in CANVAS_FORMATS:
            info['format_name'] = CANVAS_FORMATS[normalized]['name']
    if readme_meta['date']:
        info['date'] = readme_meta['date']
        try:
            info['date_formatted'] = datetime.strptime(readme_meta['date'], '%Y%m%d').strftime('%Y-%m-%d')
        except ValueError:
            pass
