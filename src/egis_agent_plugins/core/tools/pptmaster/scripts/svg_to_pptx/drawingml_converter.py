...
_CONVERTERS = {
    ...
    'text': convert_text,
    'tspan': convert_text,  # bare <tspan> outside <text> (LLM-generated SVG)
    'image': convert_image,
    'g': convert_g,
...
# Mapping from offending tag → LLM-facing fix guidance. Surfaces in the
# SvgNativeConversionError message so the model can self-correct on retry
# without us silently rewriting its SVG (matches upstream ppt-master's
# whitelist + early-fail philosophy: the skill already forbids these tags
# in shared-standards.md / spec_lock_reference.md, this is the runtime echo).
_UNSUPPORTED_TAG_HINTS: dict[str, str] = {
    'use': (
        'Bare <use href="#id"> / <use xlink:href="#id"> is not supported. '
        'Icons MUST use the project placeholder syntax '
        '`<use data-icon="<library>/<name>" x="." y="." width="." height="." '
        'fill="#HEX"/>` (single self-closing tag, NO inline <defs>/<symbol>). '
        'Library examples: chunk, tabler-filled, tabler-outline, phosphor-duotone. '
        'See references/shared-standards.md § "图标".'
    ),
    'symbol': (
        '<symbol> is not supported. Do not inline-embed full icon SVG '
        '(<svg><defs><symbol/><use href="#x"/></svg>). Replace the entire '
        'icon block with a single `<use data-icon="<library>/<name>" .../>` '
        'placeholder. See references/shared-standards.md § "图标".'
    ),
    'foreignObject': (
        '<foreignObject> is not supported by DrawingML. Express the content '
        'with native SVG primitives (<text>, <rect>, <path>, ...).'
    ),
    'script': '<script> is forbidden in pptmaster SVG.',
    'style': (
        '<style> blocks are not supported. Inline all styling as element '
        'attributes (fill, stroke, font-size, ...).'
    ),
    'animate': '<animate*> elements are not supported. Remove animations.',
    'animateTransform': '<animate*> elements are not supported. Remove animations.',
    'animateMotion': '<animate*> elements are not supported. Remove animations.',
    'textPath': (
        '<textPath> is not supported. Use plain <text> with manual '
        'positioning instead.'
    ),
}


def _build_unsupported_hint(unsupported_paths: list[str]) -> str:
    """Aggregate fix guidance for all distinct offending tags in the report."""
    seen: list[str] = []
    for path in unsupported_paths:
        # Path format: '/svg[6]/g[2]/use' → last segment without index suffix
        last = path.rsplit('/', 1)[-1]
        tag = last.split('[', 1)[0]
        if tag and tag not in seen:
            seen.append(tag)
    hints = [_UNSUPPORTED_TAG_HINTS[t] for t in seen if t in _UNSUPPORTED_TAG_HINTS]
    return ' '.join(hints)
...
def convert_svg_to_slide_shapes(
    ...
    #
    # icons_dir is resolved via PPT_MASTER_SKILL_DIR (authoritative). If the
    # env var is missing we fail loud rather than silently skipping —
    # otherwise every <use data-icon="..."/> downstream would surface as
    # 'unsupported visual SVG element(s)', completely masking the real
    # configuration error.
    from svg_finalize import get_icons_dir  # type: ignore
    ...
    # Normalize standard SVG icon idiom (<defs>+<symbol>+<use href="#id">)
    # into the inline <g>+<shape> form drawingml_converter accepts. LLMs
    # trained on web SVG conventions almost always emit this pattern; the
    # finalize_svg ``normalize-icons`` step does the same on disk, so the
    # native pipeline running here stays behaviourally aligned when fed raw
    # svg_output/ inputs.
    from svg_finalize.inline_symbol_expander import (  # type: ignore
        expand_inline_symbol_uses_in_tree,
    )
    norm_expanded, norm_removed = expand_inline_symbol_uses_in_tree(root)
    if verbose and norm_expanded:
        print(
            f'  Expanded {norm_expanded} <use href="#id"> reference(s), '
            f'removed {norm_removed} <symbol>(s)'
        )
    ...
    if unsupported:
        ...
        hint = _build_unsupported_hint(unsupported)
