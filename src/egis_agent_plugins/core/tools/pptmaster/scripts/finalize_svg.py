...
Processing options:
    ...
    normalize-icons - Expand standard <defs>+<symbol>+<use href="#id"> into
                      inline <g>+<shape> so DrawingML's whitelist accepts them.
                      Machine fallback for the LLM-preferred icon idiom.
...
from svg_finalize import get_icons_dir
...
from svg_finalize.inline_symbol_expander import expand_inline_symbol_uses
...
def finalize_project(
    ...
    # Resolve via PPT_MASTER_SKILL_DIR (authoritative). Fail loud here
    # rather than silently skipping embed-icons — missing assets used to
    # cascade into downstream 'unsupported visual' errors that masked the
    ...
    # Step 3: Normalize standard SVG icon idiom into DrawingML-compatible form.
    # LLMs trained on web SVG conventions almost always emit
    # <defs>+<symbol>+<use href="#id">; drawingml_converter rejects raw
    # <symbol>/<use href> as unsupported visuals. This step rewrites them
    # into inline <g>+<shape> with viewBox-aware translate/scale so the same
    # primitives the converter already supports carry the icon visuals.
    if options.get('normalize_icons'):
        if not quiet:
            safe_print("[2/5] Normalizing <use href> icon references...")
        norm_expanded = 0
        norm_removed = 0
        for svg_file in svg_final.glob('*.svg'):
            expanded, removed = expand_inline_symbol_uses(
                svg_file, dry_run=False, verbose=False,
            )
            norm_expanded += expanded
            norm_removed += removed
        if not quiet:
            if norm_expanded > 0:
                safe_print(
                    f"      {norm_expanded} <use href> reference(s) expanded, "
                    f"{norm_removed} <symbol>(s) removed"
                )
            else:
                safe_print("      No <use href> references")
    ...
    if options.get('align_images'):
        if not quiet:
            safe_print("[3/5] Aligning + embedding images...")
        img_count = 0
        img_errors = 0
    ...
    if options.get('flatten_text'):
        if not quiet:
            safe_print("[4/5] Flattening text...")
        flatten_count = 0
        for svg_file in svg_final.glob('*.svg'):
    ...
    if options.get('fix_rounded'):
        if not quiet:
            safe_print("[5/5] Converting rounded rects to Path...")
        rounded_count = 0
        for svg_file in svg_final.glob('*.svg'):
...
Processing options (for --only):
  ...
  normalize-icons Expand <use href="#id"> + <symbol> into inline <g>+<shape>
  ...
  crop-images, fix-aspect, embed-images  → all map to align-images
    ...
    if args.only:
        ...
        options = {
            ...
            'normalize_icons': 'normalize-icons' in only,
