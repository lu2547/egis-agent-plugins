...
Placeholder syntax (new SVGs must include a library prefix):
    <use data-icon="chunk/rocket" x="100" y="200" width="48" height="48" fill="#0076A8"/>
...
Legacy compatibility accepted by the resolver:
    <use data-icon="rocket" .../> -> chunk/rocket
    <use data-icon="chunk-filled/rocket" .../> -> chunk/rocket
...
Icon libraries (subdirectories of templates/icons/):
    chunk/             - 640+ fill icons, 16x16 viewBox  (use prefix: chunk/name; legacy 'chunk-filled/' also accepted)
...
# Default icon directory. Lazy-resolved via svg_finalize.get_icons_dir()
# so that module import does not require PPT_MASTER_SKILL_DIR to be set
# (only the CLI path needs it). Importers that need the path should call
# resolve_default_icons_dir() at the moment of use.
DEFAULT_ICONS_DIR: Path | None = None


def resolve_default_icons_dir() -> Path:
    """Lazy wrapper around :func:`svg_finalize.get_icons_dir`."""
    from svg_finalize import get_icons_dir  # local import avoids cycles
    return get_icons_dir()

# Icon base size per library
ICON_BASE_SIZES = {
    'chunk': 16,
    'chunk-filled': 16,   # backward compat alias → chunk/
    'tabler-filled': 24,
    'tabler-outline': 24,
...
def resolve_icon_path(icon_name: str, icons_dir: Path) -> tuple[Path, float]:
    ...
    Supports:
      ...
      - "chunk-filled/home"     → icons_dir/chunk/home.svg (backward compat alias)
    ...
    """
    # Backward compat: 'chunk-filled/name' → 'chunk/name' (egis-agents uses 'chunk/' directory)
    _LIB_ALIASES = {'chunk-filled': 'chunk'}
    ...
    if '/' in icon_name:
        ...
        # Backward compatibility: un-prefixed names fall back to chunk/ library
...
Examples:
  ...
  python3 scripts/svg_finalize/embed_icons.py --icons-dir my_icons/ output.svg
    ...
    parser.add_argument('--icons-dir', type=Path, default=None,
                        help='Icon directory path (default: $PPT_MASTER_SKILL_DIR/templates/icons)')
    ...
    # Resolve --icons-dir lazily so that simply importing this module never
    # raises (only CLI usage requires the directory to exist).
    if args.icons_dir is None:
        try:
            args.icons_dir = resolve_default_icons_dir()
