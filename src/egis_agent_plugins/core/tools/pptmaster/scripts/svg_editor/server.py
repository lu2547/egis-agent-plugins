...
# scripts/ root — needed so we can import the svg_finalize *package* (for
# get_icons_dir), not just its sibling modules via _FINALIZE_DIR.
_SCRIPTS_ROOT = _SCRIPTS_DIR.parent
...
from svg_finalize import get_icons_dir  # noqa: E402

# Lazy: only call get_icons_dir() when actually serving the editor (CLI
# main), so simply importing this module under test never raises.
_ICONS_DIR: Path | None = None


def _ensure_icons_dir() -> Path:
    global _ICONS_DIR
...
def _inline_icons(content: str) -> str:
    ...
    for match in reversed(matches):
        ...
        try:
            ...
            if not icon_name:
                continue
            icon_path, _ = resolve_icon_path(str(icon_name), _ensure_icons_dir())
